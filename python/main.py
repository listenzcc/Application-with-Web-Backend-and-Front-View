...
from hysplit_backward.simulate import (
    simulate_with_hysplit_backward,
    get_hysplit_backward_simulation_result,
    get_hysplit_backward_simulation_template,
    list_hysplit_backward_simulations,
    simulation_dir as hysplit_backward_simulation_dir,
)
...
@app.get('/map_backtrack')
def show_map_backtrack(lat: str = '39.906217', lon: str = '116.3912757', zoom: str = '4', session: str = '???'):
    """向前追溯的结果地图页，与 /map 同构，接口指向 hysplitBackward。"""
    print(f'Loading map_backtrack, {lat=}, {lon=}, {zoom=}, {session=}')
    html_content = Path('static/html/map_backtrack.html').read_text(encoding='utf-8')
    changes = {
        '"{{zoom}}"': zoom,
        '"{{lat}}"': lat,
        '"{{lon}}"': lon,
...
# ---------------------------------------------------------------------------
# hysplit backward（向前追溯：扩散时间为负，hycs_std 沿时间轴向过去积分）
@ui.page('/get_hysplit_backward_simulation_result/{session}')
async def get_hysplit_backward_simulation_result_page(session: str):
    """一次追溯的完整状态：status / frames / environment / config。"""
    obj = get_hysplit_backward_simulation_result(session)
    return HTMLResponse(json.dumps(obj, ensure_ascii=False),
                        media_type='application/json')


@ui.page('/get_hysplit_backward_simulation_frame')
async def get_hysplit_backward_simulation_frame_page(session: str, frame: str):
    if any(e in frame for e in ('/', '\\', '..')):
        return HTMLResponse('Bad request', status_code=400)
    try:
        p = hysplit_backward_simulation_dir(session) / 'img' / frame
    except ValueError:
        return HTMLResponse('Bad request', status_code=400)
    if not p.is_file():
        return HTMLResponse('File not found', status_code=404)
...
@ui.page('/get_hysplit_backward_simulation_table_json')
async def get_hysplit_backward_simulation_table_json_page(session: str):
    try:
        p = hysplit_backward_simulation_dir(session) / 'table.json'
    except ValueError:
        return HTMLResponse(json.dumps({}), media_type='application/json')
...
@ui.page('/get_hysplit_backward_simulation_template')
async def get_hysplit_backward_simulation_template_page(session: str):
    """把该次追溯实际用的 CONTROL 传回去（前端可查看原文）。"""
    text = get_hysplit_backward_simulation_template(session)
    if not text:
        return HTMLResponse('Not found', status_code=404)
    return HTMLResponse(text, media_type='text/plain; charset=utf-8')
...
@ui.page('/simulationBackward')
@with_layout_full_width
async def simulation_page_backward():
    with ui.row().classes('w-[1200px] justify-center items-end gap-2'):
        simulate_button = ui.button(
            '开始追溯模型计算', icon='play_arrow').props('color=primary')
    ...
    def refresh_status(session='???'):
        ...
        if not session or session == '???':
            status_label.text = ''
            return
        info = get_hysplit_backward_simulation_result(session)
        mapping = {'success': '完成', 'failed': '失败', 'pending': '计算中'}
        note = (info.get('note') or '').replace('\n', ' · ')
        status_label.text = (
            f"{session} · {mapping.get(info['status'], info['status'])}"
    ...
    def session_options():
        mapping = {'success': '完成', 'failed': '失败', 'pending': '计算中'}
        options = {}
        for entry in list_hysplit_backward_simulations():
            state = mapping.get(entry['status'], entry['status'])
            options[entry['session']] = (
                f"{entry['session']}  ·  {state}  ·  {entry['n_frames']} 帧"
                + value_range_suffix(entry))
    ...
    def on_click():
        ...
        session = simulate_with_hysplit_backward(sensors, config=collect_config())
        update_map(session=session)
        update_simulation_history()
        refresh_status(session)
    ...
    with weather_card:
        ...
        with ui.grid(columns=2).classes('w-full gap-1'):
            ...
            start_hour_input = ui.number(
                '起始时', value=12, min=0, max=23, step=1, precision=0
            ).props('dense outlined')

        duration_input = ui.number(
            '扩散时间 (h，负数=向前追溯)', value=-12, min=-240, max=240, step=1, precision=0
        ).classes('w-full mt-1').props('dense outlined')
    ...
    def update_map(session='???'):
        ...
        if selected_location in geo_candidates:
            ...
            var iframe = document.getElementById('map-iframe');
            if (iframe) {{
                iframe.src = '/map_backtrack?{params}';
            }}
            """
            ui.run_javascript(js_code)
